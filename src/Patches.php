  ...
  protected function getAndApplyPatch(HttpDownloader $downloader, $install_path, $patch_url, PackageInterface $package) {
    ...
    if (!$patched) {
      foreach ($patch_levels as $patch_level) {
        ...
        echo sprintf("Executing the follwing patch command: patch %s --no-backup-if-mismatch -d %s -i %s; \n", $patch_level, $install_path, $filename);
        if ($patched = $this->executeCommand("patch %s --no-backup-if-mismatch -d %s -i %s", $patch_level, $install_path, $filename)) {
          break;
        }
      }
    }
